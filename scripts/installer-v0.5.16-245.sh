        #!/bin/sh
        # Version-specific installer for openwrt-ha-vrrp 0.5.16-245
        set -eu
        DESTROOT="${DESTROOT:-/}"
        TARGET="0.5.16-245"
        SERIES="0.5.16"
...
version_ge() { set -- $(version_to_nums "$1") $(version_to_nums "$2")
  ...
  [ $4 -ge $8 ]
        ...
        do_install_files() {
          ...
          mkdir -p "$DESTROOT/usr/lib/ha-vrrp"; echo "0.5.16-245" > "$DESTROOT/usr/lib/ha-vrrp/VERSION"
        }
        main() { info "Installiere 0.5.16-245"; backup_config; run_pre_migrations; do_install_files; refresh_luci; info "Fertig: 0.5.16-245"; }
        main "$@"
