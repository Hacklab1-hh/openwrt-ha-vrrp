...
# genpatch004.sh — Erzeuge Patch/Diff für v0.5.16-004 aus v0.5.16-002
...
build_patch_and_diff() {
  ...
  (
    cd "$WORK_DIR"
    # Geänderte Dateien (Header-Zeilen): "diff -ruN old/... new/REL"
    diff -ruN old new 2>/dev/null \
      | while IFS= read -r line; do
          case "$line" in
            "diff -ruN "*)
              rel="${line#* new/}"     # alles bis " new/" weg
              printf '%s\n' "$rel"
              ;;
            "Only in new/"*)
              # Format: "Only in new/PFAD: DATEINAME"
              l="${line#Only in new/}"       # "PFAD: DATEINAME"
              dir="${l%%:*}"                 # "PFAD"
              file="${l#*: }"                # "DATEINAME"
              printf '%s/%s\n' "$dir" "$file"
              ;;
...
main() {
  ...
  build_patch_and_diff
  [ $BUILD_FULL -eq 1 ] && build_full_archives || true
