#!/bin/sh
# Installer for openwrt-ha-vrrp 0.5.16-007_reviewfix15
set -eu

HERE="$(cd "$(dirname "$0")" && pwd)"
. "$HERE/tools/common.sh"

VERSION="0.5.16-007_reviewfix15"
DESTROOT="${DESTROOT:-/}"
...
# Install phase: copy files from ha-vrrp/files if present
...
if [ -d "$SRC1" ]; then
  ...
  # fallback: use root relative 'usr' etc
  SRC="$PKG_ROOT"
fi
