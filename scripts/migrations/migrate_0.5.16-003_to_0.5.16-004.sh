#!/bin/sh
# scripts/migrations/migrate_0.5.16-003_to_0.5.16-004.sh – auto-generated from upgradepath.unified.json
set -eu
...
do_migrate(){
  inf "MIGRATION 0.5.16-003 → 0.5.16-004"
  mk_snapshot "pre-mig-0.5.16-003_to_0.5.16-004" /etc/config/ha_vrrp /usr/lib/ha-vrrp /luci-app-ha-vrrp || true
  ...
  ok "Migration 0.5.16-003 → 0.5.16-004 abgeschlossen."
}
do_rollback(){
  inf "ROLLBACK 0.5.16-004 → 0.5.16-003"
  mk_snapshot "pre-rb-0.5.16-004_to_0.5.16-003" /etc/config/ha_vrrp /usr/lib/ha-vrrp /luci-app-ha-vrrp || true
  apply_file_moves "$FILE_MOVES_RB"
  apply_uci_renames "$UCI_RENAMES_RB"
  unset_uci_keys "$UCI_UNSET_ON_RB"; uci_commit_ha
  ok "Rollback 0.5.16-004 → 0.5.16-003 abgeschlossen."
}
parse_args "$@"
