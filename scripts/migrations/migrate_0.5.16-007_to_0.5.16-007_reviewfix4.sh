#!/bin/sh
# scripts/migrations/migrate_0.5.16-007_to_0.5.16-007_reviewfix4.sh
# Auto-generated stub for bidirectional migration 0.5.16-007 ⇄ 0.5.16-007_reviewfix4
...
# keys to delete that were newly introduced in 0.5.16-007_reviewfix4
EOF
)
# ---------------------------------------------------------------
...
do_migrate() { 
  inf "MIGRATION 0.5.16-007 → 0.5.16-007_reviewfix4"
  mk_snapshot "pre-mig-0.5.16-007_to_0.5.16-007_reviewfix4" /etc/config/ha_vrrp /usr/lib/ha-vrrp /luci-app-ha-vrrp || true
  ...
  ok "Migration 0.5.16-007 → 0.5.16-007_reviewfix4 abgeschlossen."
}

do_rollback(){
  inf "ROLLBACK 0.5.16-007_reviewfix4 → 0.5.16-007"
  mk_snapshot "pre-rb-0.5.16-007_reviewfix4_to_0.5.16-007" /etc/config/ha_vrrp /usr/lib/ha-vrrp /luci-app-ha-vrrp || true
  apply_file_moves "$FILE_MOVES_RB"
  apply_uci_renames "$UCI_RENAMES_RB"
  unset_uci_keys "$UCI_UNSET_ON_RB"; uci_commit_ha
  ok "Rollback 0.5.16-007_reviewfix4 → 0.5.16-007 abgeschlossen."
}
