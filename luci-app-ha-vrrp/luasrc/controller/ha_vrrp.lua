...
function apply()
  ...
  http.prepare_content("application/json"); http.write_json({ ok = ok })
end
function discover()
  local http = require "luci.http"; local out = run("/usr/libexec/ha-vrrp/discover_peers.sh")
  local list = {}; for ip in out:gmatch("([%d%.]+)") do list[#list+1] = ip end
  http.prepare_content("application/json"); http.write_json({ peers = list })
end
function keysync()
  local http = require "luci.http"
...
local function dev_for(sec, uci)
  ...
  local dev = iface; if use_vlan == "1" and vid ~= "" then dev = iface .. "." .. vid end
  return dev
end
local function vip_for(sec, uci) local vip = uci:get("ha_vrrp", sec, "vip_cidr") or ""; return vip:gsub("/%d+$","") end
local function vip_present_local(dev, vip) local out = run("ip addr show dev " .. dev); if out:match("%f[%d]"..vip:gsub("%.","%%.").."%f[^%d]") then return true end; return false end
local function vip_present_remote(host, user, port, dev, vip)
  local cmd = string.format("ssh -o StrictHostKeyChecking=no -p %s %s@%s ip addr show dev %s", port or "22", user or "root", host, dev)
  local out = run(cmd); if out:match("%f[%d]"..vip:gsub("%.","%%.").."%f[^%d]") then return true end; return false
end
function statusjson()
  ...
  local peer_host = uci:get("ha_vrrp","core","peer_host") or ""; local peer_user = uci:get("ha_vrrp","core","peer_user") or "root"; local peer_port = uci:get("ha_vrrp","core","peer_port") or "22"
  local instances = {}
  uci:foreach("ha_vrrp", "instance", function(s)
    local sec = s[".name"]; local name = s.name or sec; local dev = dev_for(sec, uci); local vip = vip_for(sec, uci)
    local local_master = (dev ~= "" and vip ~= "" and vip_present_local(dev, vip)) or false
    instances[#instances+1] = { sec = sec, name = name, dev = dev, vip = vip, local_master = local_master }
  end)
  local peer_instances = {}
  if peer_host ~= "" then
    for _,it in ipairs(instances) do
      local remote_master = false; if it.dev ~= "" and it.vip ~= "" then remote_master = vip_present_remote(peer_host, peer_user, peer_port, it.dev, it.vip) end
      peer_instances[#peer_instances+1] = { sec = it.sec, name = it.name, dev = it.dev, vip = it.vip, remote_master = remote_master }
    end
  end
  http.prepare_content("application/json"); http.write_json({ node = host, peer = peer_host, local_instances = instances, peer_instances = peer_instances })
end
